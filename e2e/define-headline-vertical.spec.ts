...
//
// vertical-rl の breakpoints は window.innerHeight 比較 (root font-size 16px 基準):
//   N=8: 2240px 以上 / N=4: 960px 以上 / N=2: 640px 未満
...
// main-title は全 N で char 1 / row 1-3 に固定 (king は全 N で char >= 2 のため char 軸で分離)。
const VIEWPORTS = [
  { name: 'tall-desktop', viewport: { width: 1280, height: 2300 }, expectedN: 8 },
  { name: 'desktop', viewport: { width: 1280, height: 1080 }, expectedN: 4 },
  { name: 'mobile', viewport: { width: 375, height: 600 }, expectedN: 2 },
];

for (const v of VIEWPORTS) {
  test.describe(`defineHeadlineVertical (${v.name})`, () => {
    ...
    test(`main-title は viewport 内に visible で表示される + 衝突なし + スクショ保存 (N=${v.expectedN})`, async ({
      ...
      // h1 glyph が縦書きで縦に積まれている (writing-mode 継承が効いている) ことを実測で担保する。
      // 5 glyph の縦積みなので range box は縦長になる。
      if (!rangeBox) throw new Error('main-title の glyph range box が取得できない');
      expect(
        rangeBox.height,
        `main-title glyph box が縦長でない (writing-mode 継承漏れの疑い): ${JSON.stringify(rangeBox)}`,
      ).toBeGreaterThan(rangeBox.width);
      ...
      await page.screenshot({
        path: `e2e/_screenshots/define-headline-vertical-${v.name}.png`,
        clip,
      });
    });
    ...
    test(`全 obstacle は本文 flow text と交差しない (N=${v.expectedN})`, async ({ page }) => {
      ...
      // ずれて本文が obstacle に流入する事故を検出する (横書き demo の main-title + king @ N=2
      // で顕在化した問題の縦書き回帰網)。
